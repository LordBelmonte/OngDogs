...
(function(){
  ...
  if (!form) return console.error("Form #form-cadastro não encontrado na página.");

  form.addEventListener("submit", async function(e) {
    e.preventDefault();

    // captura segura dos campos — tenta pegar por id e por name
    function valor(id, name) {
      const elById = document.getElementById(id);
      if (elById) return elById.value;
      const elByName = form.querySelector(`[name="${name}"]`);
      return elByName ? elByName.value : null;
    }

    const dados = {
      nome: valor("nome","nome"),
      email: valor("email","email"),
      senha: valor("senha","senha"),
      telefone: valor("telefone","telefone"),
      // tenta pegar cpf tanto por id "cpf" quanto por name "cpf" ou "CPF"
      cpf: valor("cpf","cpf") ?? valor("cpf","CPF") ?? "",
    ...
    try {
      ...
      // tenta parsear JSON; se falhar, mostra aviso
      try {
        ...
        if (!json.erro) {
          ...
          alert("Erro na API: " + json.mensagem);
        }
      } catch (err) {
        console.warn("Resposta não é JSON válido.", err);
        alert("Resposta do servidor inválida. Veja console (Network e Console).");
      }

    } catch (err) {
      console.error("DEBUG -> Erro no fetch:", err);
      alert("Erro ao conectar com a API. Veja console para detalhes.");
    }
  });
})();
</script>
